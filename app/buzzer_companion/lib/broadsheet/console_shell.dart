...
class _ConsoleShellState extends State<ConsoleShell> {
  // On demarre sur « Partie », qui commence par le choix du jeu.
  //
  // Le buzzer garde en memoire le dernier jeu joue, mais l'application ne le
  // reprend plus a son compte : le jeu retenu est le sien, et il est vide
  // tant que personne n'a choisi pour cette soiree-ci.
  ...
  Widget build(BuildContext context) {
    switch (section) {
      case ConsoleSection.buzzers:
        return BuzzersScreen(
            game: game, sound: sound, ble: ble, teams: teams, tirage: tirage);
      ...
      child: Column(
        ...
        children: [
          ...
          if (isAtConfigurationMenu(game.phase)) ...[
            ...
            if (mode.isEmpty)
              OutlinedButton(
                onPressed: () => onNavigate(ConsoleSection.partie),
                style: OutlinedButton.styleFrom(
                  foregroundColor: BSColors.text,
                  side: const BorderSide(color: BSColors.divider),
                  shape: const RoundedRectangleBorder(borderRadius: BorderRadius.zero),
