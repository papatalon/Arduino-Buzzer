...
Les sons sont organisés en 6 dossiers sur la carte SD (voir [Mp3.h](Mp3.h)) : init, buzzers, bonnes réponses, mauvaises réponses, ambiance, tirage au sort. Le firmware demande bien au démarrage combien de fichiers contient chaque dossier (`readFileCountsInFolder`), mais **le module installé ne répond jamais à cette requête**, comme beaucoup de DFPlayer clones : en pratique, ce sont les constantes `*_FILE_COUNT` de [Mp3.h](Mp3.h) qui font foi. **Il faut donc les mettre à jour et reflasher chaque fois qu'on ajoute ou retire un son**, sinon les fichiers au-delà du compte inscrit ne sortiront jamais au tirage. Les comptes sont affichés sur le port série au démarrage.
